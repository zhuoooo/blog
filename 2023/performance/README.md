...
在 "计时 "（Timing）选项卡中可能看到的每个阶段的更多信息。

#### 资源调度

- 排队（**Queueing**）。浏览器在以下情况下对请求进行排队。

  - 有更高优先级的请求。

  - 已经有六个TCP连接为这个来源打开，这是极限。仅适用于HTTP/1.0和HTTP/1.1。

  - 浏览器正在短暂地分配磁盘缓存中的空间

#### 开始连接

- 停顿（**Stalled**）。该请求可能因排队中描述的任何原因而停滞。
- DNS查询（**DNS Lookup**）。浏览器正在解析该请求的IP地址。
- 初始连接（**Initial connection**）。浏览器正在建立连接，包括TCP握手/重试和谈判SSL。
- 代理协商（**Proxy negotiation**）。浏览器正与一个代理服务器协商请求。

#### 请求/响应

- 发送请求（**Request sent**）。正在发送请求。
- ServiceWorker 准备（**ServiceWorker Preparation**）。浏览器正在启动服务工作者。
- 向 ServiceWorker 发出请求（**Request to ServiceWorker**）。正在向服务工作者发送请求。
- 等待（**Waiting**）（TTFB）。浏览器正在等待响应的第一个字节。TTFB代表第一字节时间（Time To First Byte）。这个时间包括一个往返的延迟和服务器准备响应的时间。
- 内容下载（**Content Download**）。浏览器正在接收响应，可以直接从网络或从服务工作者那里接收。这个值是阅读响应主体的总时间。比预期值大，可能表明网络速度慢，或者浏览器正忙于执行其他工作，从而延迟了响应的读取。
- 接收推送（**Receiving Push**）。浏览器正在通过HTTP/2服务器推送接收该响应的数据。
