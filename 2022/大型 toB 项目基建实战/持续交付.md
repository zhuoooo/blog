...
1. 通过门禁后，将前端代码打包，获得一个 `platform.tar.gz` (包括`dist`、`Makefile`、`Dockerfile`、`chart`)；
2. 获取 `version`、 `dist` 和 `Dockerfile`，进行构建镜像，然后推送到远程仓库；
2. 获取 `version`、`chart` 进行 helm 包构建，然后推送到远程仓库；
2. （可选）构建完之后，结合 `Lighthouse` 对每次的包进行一次性能分析；
3. 测试接收到新包的通知，进行一个包部署，验证功能；
3. 最后发布
...
在 `Unix` 环境下，存在 `Makefile`  的当前目录执行
